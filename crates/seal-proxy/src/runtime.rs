...
pub struct MetricsPushRuntime {
    pub metric_push_handle: JoinHandle<anyhow::Result<()>>,
    // INV: Runtime must be dropped last.
    pub runtime: Runtime,
}

impl MetricsPushRuntime {
    /// Starts a task to periodically push metrics to a configured endpoint
    /// if a metrics push endpoint is configured.
    pub fn start(mp: EnableMetricsPush, registry: Registry) -> anyhow::Result<Self> {
        ...
        let metric_push_handle = tokio::spawn(async move {
            let mut interval = tokio::time::interval(mp.config.push_interval);
            interval.set_missed_tick_behavior(tokio::time::MissedTickBehavior::Skip);
            let mut client = create_push_client();
            tracing::info!("starting metrics push to '{}'", &mp.config.push_url);
            loop {
                tokio::select! {
                    _ = interval.tick() => {
...
pub async fn push_metrics(
    ...
    tracing::info!("metric_families: {:?}", metric_families);
